...
struct Dyadic {
    ...
    V twists,untwists,twist_quotients,untwist_quotients;
    Dyadic(unsigned length,U prime,U gen):n(length),p(prime),
        ...
        twists(n),untwists(n),twist_quotients(n),untwist_quotients(n) {
    ...
    V run(const V& x,bool inverse) const {
        ...
        if(inverse){V y=backward.run(x); for(unsigned j=0;j<n;++j)y[j]=shoup(y[j],untwists[j],untwist_quotients[j],p);return y;}
        V y=x;for(unsigned j=1;j<n;++j)y[j]=shoup(y[j],twists[j],twist_quotients[j],p);return forward.run(y);
    }
};
...
struct Normal {
    ...
    NTT forward,backward;
    std::array<V,2> kernels,kernel_quotients;
    ...
    V run(const V& x,bool inverse) const {
        ...
        for(unsigned j=0;j<n;++j)spectrum[j]=shoup(spectrum[j],kernels[inverse][j],kernel_quotients[inverse][j],p);
    ...
    V transform(const U* source,unsigned channel,bool inverse,unsigned len=0) {
        ...
        auto extension=[&](){
            ...
            for(unsigned j=0;j<n;++j)for(unsigned i=0;i<len;++i) {
                ...
                tensor[j*len+i]=shoup(x,d.kernels[inverse][j],d.kernel_quotients[inverse][j],p);
            }
            for(unsigned j=0;j<n;++j) {
        ...
        auto first_axis=[&](){
            ...
            for(unsigned j=0;j<len;++j) {
                ...
                for(unsigned e=0;e<256;++e)
                    tensor[j*256+e]=twist==1?out[i*256+e]:shoup(out[i*256+e],twist,d.twist_quotients[i],p);
            }
            unsigned span=2,stage=0;
            for(const auto& tw:t.twiddles) {
                const auto& quot=t.shoup_quotients[stage++];
            ...
            for(unsigned i=0;i<len;++i)for(unsigned e=0;e<256;++e) {
                ...
                out[i*256+e]=inverse?shoup(x,d.untwists[i],d.untwist_quotients[i],p):x;
            }
        };
