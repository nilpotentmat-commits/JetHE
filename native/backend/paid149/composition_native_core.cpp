...
static U mul(U a, U b, U p) { return U(Wide(a)*b%p); }
    ...
    V run(const V& input) const {
        ...
        for(const auto& tw:twiddles) {
            for(unsigned start=0;start<n;start+=span) for(unsigned j=0;j<span/2;++j) {
                U a=out[start+j],b=out[start+j+span/2]; if(j)b=mul(b,tw[j],p);
                out[start+j]=add(a,b,p); out[start+j+span/2]=sub(a,b,p);
            }
            span*=2;
        }
        return out;
    ...
    V transform(const U* source,unsigned channel,bool inverse,unsigned len=0) {
        ...
        auto extension=[&](){
            for(unsigned i=0;i<len;++i) {
                ...
                V y=normal[channel]->run(scratch,inverse);std::copy(y.begin(),y.end(),out.begin()+i*256);
            }
        };
        auto first_axis=[&](){
            scratch.resize(len);
            for(unsigned e=0;e<256;++e) {
                ...
                V y=conventional?first[channel]->run(scratch,inverse):axis(channel,len).run(scratch,inverse);
...
API int jc_digits(void* handle,const U* in,I* out,unsigned limbs,unsigned width) {
    BEGIN auto& c=*static_cast<Context*>(handle);c.channels(limbs);need(width==48||width==60,"supported gadget widths48/60");
