// Native HElib terminal-prime policy; prior drivers/failures are preserved.
...
struct NoiseAdmissionReject:std::runtime_error {
  NoiseAdmissionReject():std::runtime_error("HElib public noise admission rejected before decryption") {}
};
struct Timer {
...
public:
  ...
  void observe(const helib::Ctxt& c,unsigned parts,unsigned group,const std::string& name,bool inspect=false) {
    ...
    if(!c.isCorrect()) {
      std::cout<<Json({{"event","public_noise_admission_rejection"},{"group",group},
        {"operation",name},{"parts",parts},{"prime_indices",indices(c.getPrimeSet())},
        {"bit_capacity",capacity},{"library_is_correct",false}}).dump()<<'\n'<<std::flush;
      throw NoiseAdmissionReject();
    }
  }
  ...
  void normalize(helib::Ctxt& a) {
    // Small primes are legal terminal moduli; do not force a ctxt-prime carrier.
    down(a,a.getPrimeSet()/context.getSpecialPrimes());
  }
  void add(helib::Ctxt& a,helib::Ctxt b) {
    // Preserve the accumulator's modulus, including legal small primes.
    // bringToSet can introduce a prime before dropping another; charge it.
    if(a.getPrimeSet()!=b.getPrimeSet()) {
      const auto before=b.getPrimeSet(),target=a.getPrimeSet();
      { Timer t(times,"evaluation_modulus"); b.bringToSet(target); }
      ++ledger.explicit_mod_changes;
      require(b.getPrimeSet()==target,"Addend modulus alignment");
      if(gate) ledger.checkpoints.push_back({{"operation","addend_bring_to_accumulator"},
        {"before_prime_indices",indices(before)},{"prime_indices",indices(target)}});
    }
  ...
  helib::Ctxt primary(std::vector<helib::Ctxt> in,const core::Cell& c,const Policy& p,unsigned group) {
    ...
    for(unsigned i=0;i<(c.family==3?4u:1u);++i) {
      ...
      if(c.family==3&&p.first) { relin(value,3); normalize(value); observe(value,2,group,"level1_relin",group==0&&i==0); }
    ...
    while(level.size()>1) {
      ...
      for(unsigned i=0;i<level.size();i+=2) {
        ...
        if(depth==2&&p.second) { relin(value,raw_parts); normalize(value); observe(value,2,group,"level2_relin",group==0&&i==0); }
    ...
    normalize(level[0]); observe(level[0],parts,group,"terminal",true); return std::move(level[0]);
  }
};
...
static Json batch(const core::Cell& c,const std::string& arm,const Policy& p,
  ...
  if(arm=="crt") {
    ...
    for(unsigned block=0;block<8;++block) {
      ...
      require(raw&&canonical,"Incomplete BSGS groups"); eval.normalize(*raw); eval.normalize(*canonical); eval.add(*raw,std::move(*canonical));
...
int main(int argc,char** argv) {
  try {
    ...
    std::cout<<Json({{"status","CORE_CONVENTIONAL_NOISE_REJECT"},{"reason",e.what()},
      {"full_output_verified",false},{"encrypted_execution",true},{"benchmark",false},
      {"security_128_qualified",false}}).dump()<<'\n'; return 2;
