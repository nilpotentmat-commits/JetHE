"""V3 actual-public-modulus screen and v2 static-log containment.
...
PAPER = HERE.parent
ROOT = PAPER.parent
BINARY = PAPER / "build/helib-terminal-core-v2/helib_terminal_core"
PROTOCOL = HERE / "CORE_CONVENTIONAL_GATE_V3.md"
PREFIX = "core-conventional-v3-"
...
def manifest():
    ...
    paths.update([PROTOCOL, HERE / "CORE_GRID_V1_DESIGN.md", HERE / "CORE_GRID_V1_GATE_PROTOCOL.md",
        ...
        HERE / "helib_composition/core/core_math.h", HERE / "helib_composition/core/terminal_core.cpp", HERE / "helib_composition/core/terminal_core_v2.cpp",
        HERE / "helib_composition/core/published_q/CMakeLists.txt",
        HERE / "CORE_CONVENTIONAL_GATE_V1.md", HERE / "CORE_CONVENTIONAL_GATE_V2.md",
        PAPER / "build/helib-terminal-core-v2/CMakeCache.txt", PAPER / "build/helib-terminal-core-v2/build.ninja",
...
def main():
    ...
    work_root = PAPER / "build/core-conventional-gate-v3"
    work_root.mkdir(exist_ok=True)
    lock = (work_root / "campaign.lock").open("a+")
    fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    existing = [p for version in (1, 2, 3) for p in (PAPER / "evidence").glob(f"core-conventional-v{version}-*.json")]
    ...
    receipt = dict(schema="core-conventional-functional-v3", status=status, label=label, mode=mode,
