"""Bounded, immutable HElib terminal-core public checks and encrypted admission.
...
PAPER = HERE.parent
ROOT = PAPER.parent
BINARY = PAPER / "build/helib-terminal-core-v1/helib_terminal_core"
PROTOCOL = HERE / "CORE_CONVENTIONAL_GATE_V1.md"
PREFIX = "core-conventional-v1-"
...
def manifest():
    ...
    paths.update([PROTOCOL, HERE / "CORE_GRID_V1_DESIGN.md", HERE / "CORE_GRID_V1_GATE_PROTOCOL.md",
        ...
        HERE / "helib_composition/core/core_math.h", HERE / "helib_composition/core/terminal_core.cpp",
        HERE / "helib_composition/core/CMakeLists.txt",
        PAPER / "build/helib-terminal-core-v1/CMakeCache.txt", PAPER / "build/helib-terminal-core-v1/build.ninja",
    ...
    paths.update(backend / p for p in ("src/Ctxt.cpp", "src/keys.cpp", "src/EncryptedArray.cpp",
        "src/DoubleCRT.cpp", "src/JsonWrapper.cpp", "include/helib/Ctxt.h",
...
def validate_result(result, mode, arm=None, cell=None, m=None, bits=None, policy=None):
    ...
    assert profile["library_128_screen"] and profile["library_security_estimate_NOT_CERTIFICATION"] >= 128
...
def main():
    ...
    work_root = PAPER / "build/core-conventional-gate-v1"
    work_root.mkdir(exist_ok=True)
    lock = (work_root / "campaign.lock").open("a+")
    fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    existing = list((PAPER / "evidence").glob(PREFIX+"*.json"))
    ...
    try:
        ...
        assert not list(work.iterdir()), "Unexpected worker-created file"
    ...
    receipt = dict(schema="core-conventional-functional-v1", status=status, label=label, mode=mode,
        ...
        dynamic_linker_report=ldd_output, worker_cwd=work.relative_to(ROOT).as_posix(),
