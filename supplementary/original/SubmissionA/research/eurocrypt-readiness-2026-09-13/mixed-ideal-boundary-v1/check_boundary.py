...
def main():
    ...
    for q,l,h in product((2,3,4),(2,3,4,8),(1,2)):
        ...
        if l>=3:
            ...
            g0=tuple(int(j%l==1) for j in range(len(f)))
            g1=r.add(g0,g)
            assert all(g0[j*l+1]==g1[j*l+1]==1 for j in range(h))
            values=(r.compose(r.zero,g0),r.compose(f,g0),r.compose(r.zero,g1),r.compose(f,g1))
            assert mixed(r,values)==g
            unit_linear_witnesses+=1
    ...
    result=dict(status='MIXED_IDEAL_BOUNDARY_PUBLIC_CHECK_PASS',exhaustive=exhaustive,circuits=circuits,
                ...
                unit_linear_inner_witnesses=unit_linear_witnesses,
