...
sys.path[:0]=[str(HERE),str(HERE.parent/'same-algebra-one-prime-workflow-v1'),str(STAGE)]
import common
from stage_crypto import SlimRing as BaselineRing
from tensor_crypto import SlimRing as CandidateRing
...
def main():
    ...
    assert str(candidate.dll._name)==str(HERE/'build/tensor_core.so')
    ...
    for name,want in build['source_bindings'].items():assert binding(ROOT/name)==want,name
    for name,want in build['candidate_files'].items():assert binding(HERE/name)==want,name
    result=dict(status='NATIVE_TENSOR_AXIS_PUBLIC_CORRESPONDENCE_PASS',counts=dict(counts),kernel_samples=samples,
