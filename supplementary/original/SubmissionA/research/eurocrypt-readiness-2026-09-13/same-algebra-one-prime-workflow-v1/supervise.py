...
def validate_result(result,mode,arm,index):
    assert result['status']==('ONE_PRIME_CONTROL_WORKFLOW_GATE_PASS' if mode=='gate' else 'ONE_PRIME_CONTROL_WORKFLOW_WORKER_PASS')
    ...
    assert result['profile_version']=='same-algebra-one-prime-workflow-v1'
    assert result['terminal_limbs']==(2 if arm=='native' else 1)
    ...
    assert material['public_material_raw_bytes']==(391 if arm=='native' else 1)<<20
    ...
    for i,batch in enumerate(result['batches']):
        ...
        I,C,O,polys,mb=(35,19,1,3,103) if arm=='native' else (346,86,88,262,346)
        ...
        assert [wire[k] for k in ('input_ciphertexts','input_polynomials','input_raw_bytes','output_ciphertexts','output_polynomials','output_raw_bytes')]==[I,2*I,mb<<20,O,polys,out_mb<<20]
...
def main():
    ...
    p.add_argument('--host-observation',type=Path,default=HERE/'host-observation-v1.json')
    args=p.parse_args()
    verify_admission()
    ...
    if args.mode=='campaign':
        ...
        host_observation=json.loads(args.host_observation.read_text())
        assert set(host_observation)=={'wsl','windows'}
        now=datetime.now(timezone.utc)
        for observation in host_observation.values():
            assert observation['workers']==[] and observation['tool_exit_code']==0
            observed=datetime.fromisoformat(observation['recorded_utc'])
            assert 0<=(now-observed).total_seconds()<=300,'Use fresh host observations.'
    output=args.out.resolve()
    assert output.parent==HERE
    output.mkdir(exist_ok=False)
    if args.mode=='campaign':save(output/'host-observation.json',host_observation)
    ...
    if args.mode=='gate':
        save(output/'gate.json',dict(status='ONE_PRIME_CONTROL_WORKFLOW_BOTH_GATES_PASS',
        ...
        result=dict(status='ONE_PRIME_CONTROLLED_WORKFLOW_CAMPAIGN_PASS',order=list(ORDER),
