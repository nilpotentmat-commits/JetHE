"""One-prime control workflow, with a common codec and immutable inputs."""
...
ONE=READY/'same-algebra-one-prime-v1'
...
from admission import Q as NATIVE_Q,N,bindings as control_admission_bindings
from interpolation import Interpolation
from prepare import outer_prepare,inner_prepare,records,record_digest
from composition_wire import send

_one=json.loads((ONE/'admission.json').read_text())
Q,F,BRAW=_one['q'],_one['fresh_cap'],_one['raw_cap']
assert (Q,F,BRAW)==(1152921504002872321,36636,1404684555427328)
...
def source_manifest():
    dirs=(HERE,SAME,FAST,OLD,LIGHT,RESEARCH,PAIRED,ONE)
    paths={HERE/name for name in ('PLAN.md','common.py','worker.py','supervise.py','check_host_workers.py','check_driver.py')}
    paths.update(ROOT/name for name in control_admission_bindings())
    paths.add(ONE/'admission.json')
    paths.update(ROOT/name for name in _one['bindings'])
...
def verify_admission():
    ...
    candidate=json.loads((ONE/'admission.json').read_text())
    assert candidate==_one
    assert candidate['status']=='ONE_PRIME_SAME_ALGEBRA_CONDITIONAL_ADMISSION_CHECKS_PASS'
    assert not candidate['new_he_execution'] and candidate['security_bits'] is None
    assert candidate['fixed_batches']==1024 and candidate['ideal_correctness_strict_exponent']==149
    assert candidate['raw_payload_bytes']==dict(public=1<<20,inputs=346<<20,outputs=131<<20)
    for name,want in candidate['bindings'].items():assert binding(ROOT/name)==want,name
