...
def result_check(r,arm,mode,index):
    assert r['status']==('ONE_PRIME_CONTROL_WORKFLOW_GATE_PASS' if mode=='gate' else 'ONE_PRIME_CONTROL_WORKFLOW_WORKER_PASS')
    ...
    assert r['profile_version']=='same-algebra-one-prime-workflow-v1'
    ...
    I,C,O,P,MB=(35,19,1,3,103) if arm=='native' else (346,86,88,262,346)
    assert r['material']['public_material_raw_bytes']==(391 if arm=='native' else 1)<<20
    for i,b in enumerate(r['batches']):
        ...
        out_mb=3 if arm=='native' else 131
        assert (w['output_ciphertexts'],w['output_polynomials'],w['output_raw_bytes'])==(O,P,out_mb<<20)
...
def main():
    ...
    assert gate['status']=='ONE_PRIME_CONTROL_WORKFLOW_BOTH_GATES_PASS'
    source,runtime=gate['source_manifest'],gate['runtime_manifest']
    preflight=read(HERE/'preflight.json')
    assert preflight['status']=='ONE_PRIME_WORKFLOW_PUBLIC_PREFLIGHT_PASS'
    assert preflight['source_manifest']==source and preflight['runtime_manifest']==runtime
    assert not preflight['new_he_execution'] and preflight['secret_vectors_sampled']==0
    assert [(x['limbs'],x['bound']) for x in preflight['cases']]==[
        (1,36636),(1,1404684555427328),(1,288230376000718079),
        (2,36636),(2,1404684555427328)]
    assert all(x['phase_coefficients']==65536 and x['decoded_symbols']==4096
               and x['uncentered_parity_mismatches']==32768 for x in preflight['cases'])
    ...
    if args.campaign:
        host=read(args.campaign/'host-observation.json')
        ...
        assert campaign['status']=='ONE_PRIME_CONTROLLED_WORKFLOW_CAMPAIGN_PASS'
        ...
        files += [HERE/'check_host_workers.py']
    files+=[HERE/'verify_workflow.py',HERE/'RESULTS.md',HERE/'REPRODUCE.md',HERE/'preflight.json']
    result=dict(status='ONE_PRIME_CONTROL_WORKFLOW_CAMPAIGN_READBACK_PASS' if campaign else 'ONE_PRIME_CONTROL_WORKFLOW_GATES_READBACK_PASS',
        ...
        public_preflight_verified=True,public_preflight_phase_coefficients=327680,
