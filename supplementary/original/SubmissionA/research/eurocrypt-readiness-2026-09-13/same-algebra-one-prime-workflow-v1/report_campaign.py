...
def tables(receipt):
    assert receipt['status']=='ONE_PRIME_CONTROL_WORKFLOW_CAMPAIGN_READBACK_PASS'
    ...
    lines=[r'\begin{table}[ht]',r'\centering\small',r'\setlength{\tabcolsep}{4pt}',
           r'\caption{The one-prime-control campaign: median (minimum--maximum) over three fresh setups per arm. Times are seconds; RSS is MiB. All complete local batch work is charged. These are descriptive samples under the stated source assumptions, not a numerical security match.}',
           r'\label{tab:one-prime-workflow}',r'\begin{tabular}{lrr}',r'\toprule',
           r'Metric & Terminal JetHE & One-prime control\\',r'\midrule']
    ...
    detail=[r'\begin{table}[ht]',r'\centering\small',r'\setlength{\tabcolsep}{3pt}',
            ...
            r'\label{tab:one-prime-samples}',r'\begin{tabular}{rlrrrrr}',r'\toprule',
    ...
    return {'one-prime-workflow.tex':main,'one-prime-samples.tex':'\n'.join(detail)+'\n'}
...
def main():
    ...
    print(json.dumps(dict(status='ONE_PRIME_RECORDED_CAMPAIGN_TABLES_GENERATED',
