...
ORDER=('baseline','extension','extension','baseline','baseline','extension','extension','baseline')
...
def imports():
    sys.path[:0]=[str(ORIGINAL),str(LAZY),str(STAGE),str(TENSOR),str(HERE)]
    import common
    from extension_worker import ExtensionWorker
    return common,ExtensionWorker

def baseline_module():
    import importlib.util
    spec=importlib.util.spec_from_file_location('checked_tensor_driver',TENSOR/'run.py')
...
def manifest(common):
    ...
    assert build['status']=='NATIVE_EXTENSION_AXIS_BUILD_PASS'
    assert check['status']=='NATIVE_EXTENSION_AXIS_PUBLIC_CORRESPONDENCE_PASS'
    ...
    for name in ['WORKFLOW_PLAN.md','run.py','extension_worker.py','build.json','public-check.json','check_candidate.py']:
...
def runtime(common):
    ...
    result[str(HERE/'build/extension_core.so')]=binding(HERE/'build/extension_core.so')
    return result


def validate(r,mode,variant,index):
    assert r['status']=='NATIVE_EXTENSION_AXIS_WORKFLOW_'+('GATE_PASS' if mode=='gate' else 'MEASUREMENT_PASS')
    assert (r['mode'],r['variant'],r['index'],r['arm'])==(mode,variant,index,'native')
    assert r['profile_version']=='native-extension-axis-v1' and r['terminal_limbs']==2
...
def child(args):
    ...
    try:
        result=worker.run()
        result.update(status='NATIVE_EXTENSION_AXIS_WORKFLOW_'+('GATE_PASS' if args.mode=='gate' else 'MEASUREMENT_PASS'),
            variant=args.variant,profile_version='native-extension-axis-v1')
...
def main():
    ...
    p.add_argument('--variant',choices=('baseline','extension'));p.add_argument('--index',type=int)
    ...
    if args.mode=='measure':
        ...
        assert gate['status']=='NATIVE_EXTENSION_AXIS_BOTH_FRESH_GATES_PASS'
    ...
    order=('baseline','extension') if args.mode=='gate' else ORDER
    ...
    result=dict(status='NATIVE_EXTENSION_AXIS_BOTH_FRESH_GATES_PASS' if args.mode=='gate' else 'NATIVE_EXTENSION_AXIS_CONTROLLED_WORKFLOW_PASS',
        ...
        baseline_over_extension_median_ratios={k:summary['baseline'][k]['median']/summary['extension'][k]['median'] for k in summary.get('baseline',{})},
