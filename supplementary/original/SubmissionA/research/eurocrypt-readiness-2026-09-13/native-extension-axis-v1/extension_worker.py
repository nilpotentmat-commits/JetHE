...
from worker import *
from tensor_crypto import SlimRing as TensorRing
from extension_crypto import SlimRing as ExtensionRing
import stage_public

class ExtensionWorker(Worker):
    def __init__(self,mode,variant,index):
        super().__init__(mode,'native',index)
        assert variant in ('baseline','extension')
        self.variant=variant
        self.native=stage_public
        self.terminal_limbs=2

    def setup(self):
            start=perf_counter()
            with timed(self.setup_times,'public_context_codec'):
                self.ring=ExtensionRing() if self.variant=='extension' else TensorRing()
            ...
            assert self.material['public_material_raw_bytes']==297<<20
            assert coins.errors==107
