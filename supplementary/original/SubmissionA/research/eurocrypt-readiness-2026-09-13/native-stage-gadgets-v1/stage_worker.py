"""Explicit per-instance stage-gadget compilation; shared owner and recipient boundary."""
from worker import *
from candidate_crypto import SlimRing as LazyRing
from stage_crypto import SlimRing as StageRing
import stage_public

class StageWorker(Worker):
    def __init__(self,mode,variant,index):
        super().__init__(mode,'native',index)
        assert variant in ('baseline','stage')
        self.variant=variant
        self.native=stage_public if variant=='stage' else native
        self.terminal_limbs=2

    def setup(self):
            start=perf_counter()
            with timed(self.setup_times,'public_context_codec'):
                self.ring=StageRing() if self.variant=='stage' else LazyRing()
                _,_,_,_,self.rows,self.inverse=codec_setup()
                self.field=SharedField()
                assert self.ring.moduli[self.terminal_limbs]==NATIVE_Q
                if self.arm=='control':
            ...
            with timed(self.setup_times,'keys_and_hints'):
                ...
                if self.arm=='native':
                    self.keys,self.pks,self.banks=self.native.setup(self.ring,coins)
                else:
            ...
            assert self.material['public_material_raw_bytes']==(297 if self.variant=='stage' else 391)<<20
            assert coins.errors==(107 if self.variant=='stage' else 139)
    ...
    def native_batch(self,fs,gs):
            ...
            with timed(times,'evaluation'):
                evaluated=self.native.evaluate(self.ring,public,self.gate)
            ...
            if self.gate:
                ...
                for ct,(name,key,a,arity,bound) in zip(evaluated,self.native.spec(self.ring)):
