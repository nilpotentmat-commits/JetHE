...
TENSOR=READY/'native-tensor-axis-v1'
EXTENSION=READY/'native-extension-axis-v1'
ROOT=HERE.parents[3]
ORIGINAL=READY/'same-algebra-one-prime-workflow-v1'
LAZY=READY/'native-lazy-ntt-v1'
ORDER=('baseline','fixed','fixed','baseline','baseline','fixed','fixed','baseline')
...
def imports():
    sys.path[:0]=[str(ORIGINAL),str(LAZY),str(STAGE),str(TENSOR),str(EXTENSION),str(HERE)]
    import common
    from fixed_worker import FixedWorker
    return common,FixedWorker

def baseline_module():
    import importlib.util
    spec=importlib.util.spec_from_file_location('checked_extension_driver',EXTENSION/'run.py')
...
def manifest(common):
    ...
    assert build['status']=='NATIVE_FIXED_MULTIPLIER_BUILD_PASS'
    assert check['status']=='NATIVE_FIXED_MULTIPLIER_PUBLIC_CORRESPONDENCE_PASS'
    ...
    for name in ['WORKFLOW_PLAN.md','run.py','fixed_worker.py','build.json','public-check.json','check_candidate.py']:
...
def runtime(common):
    ...
    result[str(HERE/'build/fixed_core.so')]=binding(HERE/'build/fixed_core.so')
    return result


def validate(r,mode,variant,index):
    assert r['status']=='NATIVE_FIXED_MULTIPLIER_WORKFLOW_'+('GATE_PASS' if mode=='gate' else 'MEASUREMENT_PASS')
    assert (r['mode'],r['variant'],r['index'],r['arm'])==(mode,variant,index,'native')
    assert r['profile_version']=='native-fixed-multipliers-v1' and r['terminal_limbs']==2
...
def child(args):
    ...
    try:
        result=worker.run()
        result.update(status='NATIVE_FIXED_MULTIPLIER_WORKFLOW_'+('GATE_PASS' if args.mode=='gate' else 'MEASUREMENT_PASS'),
            variant=args.variant,profile_version='native-fixed-multipliers-v1')
...
def main():
    ...
    p.add_argument('--variant',choices=('baseline','fixed'));p.add_argument('--index',type=int)
    ...
    if args.mode=='measure':
        ...
        assert gate['status']=='NATIVE_FIXED_MULTIPLIER_BOTH_FRESH_GATES_PASS'
    ...
    order=('baseline','fixed') if args.mode=='gate' else ORDER
    ...
    result=dict(status='NATIVE_FIXED_MULTIPLIER_BOTH_FRESH_GATES_PASS' if args.mode=='gate' else 'NATIVE_FIXED_MULTIPLIER_CONTROLLED_WORKFLOW_PASS',
        ...
        baseline_over_fixed_median_ratios={k:summary['baseline'][k]['median']/summary['fixed'][k]['median'] for k in summary.get('baseline',{})},
