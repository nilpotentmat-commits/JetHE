"""Portable consistency checks of the completed fixed-multiplier evidence.
...
def main():
    ...
    assert campaign['status'] == 'NATIVE_FIXED_MULTIPLIER_CONTROLLED_WORKFLOW_PASS'
    assert gate['order'] == ['baseline', 'fixed'] and campaign['order'] == ORDER
    assert gate['source_manifest'] == campaign['source_manifest']
    assert gate['runtime_manifest'] == campaign['runtime_manifest']
    assert len(gate['source_manifest']) == 155 and len(gate['runtime_manifest']) == 14
    ...
    for directory, summary, mode in ((HERE / 'gate-v1', gate, 'gate'),
        ...
        for index, variant in enumerate(summary['order']):
            ...
            assert result['status'] == 'NATIVE_FIXED_MULTIPLIER_WORKFLOW_' + suffix
            assert result['profile_version'] == 'native-fixed-multipliers-v1'
    ...
    assert build['status'] == 'NATIVE_FIXED_MULTIPLIER_BUILD_PASS' and build['returncode'] == 0
    ...
    ratios = campaign['baseline_over_fixed_median_ratios']
    assert set(ratios) == set(calculated['baseline'])
    for key, ratio in ratios.items():
        assert isclose(ratio, calculated['baseline'][key]['median'] /
                       calculated['fixed'][key]['median'], rel_tol=1e-12)
    from report_results import render
    assert (HERE / 'RESULTS.md').read_text(encoding='utf-8') == render()
    files += [HERE / name for name in ('verify_records.py', 'RESULTS.md', 'REPRODUCE.md', 'report_results.py',
                                      'host-observation-v1.json', 'execution.json', 'observe_wsl.py', 'host-windows-v1.json', 'host-wsl-v1.json', 'public-execution.json', 'gate-execution.json', 'campaign-execution.json')]
    receipt = dict(status='NATIVE_FIXED_MULTIPLIER_RECORDED_WORKFLOW_READBACK_PASS',
                   source_files=len(gate['source_manifest']), recorded_runtime_files=len(gate['runtime_manifest']),
                   fresh_gate_states_by_arm=dict(baseline=57, fixed=57),
                   fresh_gate_coefficients_by_arm=dict(baseline=3735552, fixed=3735552),
