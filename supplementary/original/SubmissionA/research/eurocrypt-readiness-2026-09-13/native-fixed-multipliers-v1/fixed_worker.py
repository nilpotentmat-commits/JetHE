...
from worker import *
from extension_crypto import SlimRing as BaselineRing
from fixed_crypto import SlimRing as FixedRing
import stage_public

class FixedWorker(Worker):
    def __init__(self,mode,variant,index):
        super().__init__(mode,'native',index)
        assert variant in ('baseline','fixed')
    ...
    def setup(self):
            ...
            with timed(self.setup_times,'public_context_codec'):
                self.ring=FixedRing() if self.variant=='fixed' else BaselineRing()
