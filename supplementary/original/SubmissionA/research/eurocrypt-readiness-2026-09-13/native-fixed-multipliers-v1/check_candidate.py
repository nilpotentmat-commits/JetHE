...
EXTENSION=HERE.parent/'native-extension-axis-v1'
sys.path[:0]=[str(HERE),str(HERE.parent/'same-algebra-one-prime-workflow-v1'),str(STAGE),str(TENSOR),str(EXTENSION)]
import common
from extension_crypto import SlimRing as BaselineRing
from fixed_crypto import SlimRing as CandidateRing
...
def main():
    ...
    assert str(candidate.dll._name)==str(HERE/'build/fixed_core.so')
    ...
    for name,want in build['source_bindings'].items():assert binding(ROOT/name)==want,name
    for name,want in build['candidate_files'].items():assert binding(HERE/name)==want,name
    result=dict(status='NATIVE_FIXED_MULTIPLIER_PUBLIC_CORRESPONDENCE_PASS',counts=dict(counts),kernel_samples=samples,
        ...
        scope='Public deterministic arithmetic checks and fixed-coin kernel screen only; no new encrypted gate, complete-workflow speedup or law change. Baseline is the 102-row extension-axis implementation.')
