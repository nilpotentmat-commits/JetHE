...
ROOT=HERE.parents[3]
sys.path[:0]=[str(HERE),str(FAST),str(FAST.parent)]
from slim_crypto_v2 import SlimRing as BaselineRing
from candidate_crypto import SlimRing as CandidateRing
...
def main():
    ...
    assert str(candidate.dll._name)==str(HERE/'build/lazy_core.so')
    ...
    try:
        ...
        for a in (2,3):
            ...
            for j in range(baseline.gadget(a)):
                for sign in (-1,1):
                    for offset in (-1,0,1):
                        v=sign*((1<<43)*(1<<(44*j))+offset)
            ...
            bd=baseline.digits(spectrum,a);cd=candidate.digits(spectrum,a)
            for b,c in zip(bd,cd):equal(b.tolist(),c.tolist(),'gadget_words')
            assert all(sum(int(d[i])<<(44*j) for j,d in enumerate(cd))==value for i,value in enumerate(full))
    ...
    for name,want in build['source_bindings'].items():assert binding(ROOT/name)==want,name
    for name,want in build['candidate_files'].items():assert binding(HERE/name)==want,name
    result=dict(status='NATIVE_LAZY_NTT_BACKEND_PUBLIC_CORRESPONDENCE_PASS',counts=dict(counts),kernel_samples=samples,
        ...
        scope='Public deterministic arithmetic checks and fixed-coin kernel screen only; no new encrypted gate, complete-workflow speedup or law change.')
