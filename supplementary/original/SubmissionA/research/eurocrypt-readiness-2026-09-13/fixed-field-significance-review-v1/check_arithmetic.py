"""Finite algebra checks; not a fast-backend benchmark, HE run or proof checker."""
import hashlib
import json
...
if __name__ == '__main__':
    result = check()
    (HERE / 'arithmetic-check.json').write_text(json.dumps(result, indent=2), encoding='utf-8')
    print(json.dumps({k: len(result[k]) for k in ('ring_cases', 'jet_cases', 'carlitz_cases')}))
