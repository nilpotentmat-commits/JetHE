"""Public owner/recipient transforms for one product in the native algebra."""
...
sys.path.insert(0,str(HERE.parent/'paired-terminal-receiver-v1'))
...
class Interpolation:
    def __init__(self,field,t=128,length=256):
        ...
        self.dll=C.CDLL(str(HERE/'build/interpolation.so'))
...
def slow_evaluate(polynomial,point):
    value=0
    for c in reversed(polynomial): value=slow_mul(value,point)^c
    return value
