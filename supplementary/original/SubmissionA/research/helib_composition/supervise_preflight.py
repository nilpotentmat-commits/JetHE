"""Linux-side limits for one public HElib field preflight, not a benchmark.

Windows JobObject caps do NOT cover this WSL worker. RLIMIT_AS below is
address space, not an asserted Linux hard-RSS mechanism. The worker itself
sets NTL threads to1 and contains no fork/exec path.
"""
import hashlib
import json
import os
...
def main():
    ...
    executable=root/'build'/'helib-composition'/'helib_composition_preflight'
    ...
    child=subprocess.Popen([str(executable),'--public-codec'],cwd=executable.parent,
    ...
    try:
        ...
        if child.poll() is None:
            ...
            os.killpg(child.pid,signal.SIGKILL) # Only this owned session's group.
    ...
    if len(results)!=1 or results[0].get('status')!='HELIB_PUBLIC_FIELD_PREFLIGHT_ONLY' or not same:
    ...
    receipt=dict(status='PASS' if error is None else 'FAIL',error=error,
          ...
          scope='One Linux-side public field-codec check; no HE keys, ciphertexts or timing statistics')
    if error:receipt['diagnostic']=text[-8192:]
